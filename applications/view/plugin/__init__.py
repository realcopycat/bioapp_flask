...
def register_plugin_views(app: Flask):
    ...
    for plugin_folder in PLUGIN_ENABLE_FOLDERS:
        ...
        try:
            ...
            print(f"-->载入插件 {plugin_info['plugin_name']} 中......")
            __import__('plugins.' + plugin_folder + '.main')
            print(f"-->载入插件 {plugin_info['plugin_name']} 成功......")
        except BaseException as e:
            info = f"-->载入插件时{plugin_info['plugin_name'] if len(plugin_info) != 0 else ''}出现下列错误：" + "\n"
    ...
    if plugin_folder_name:
        try:
            if plugin_folder_name not in PLUGIN_ENABLE_FOLDERS:
                ...
                # 启用插件事件
                try:
                    importlib.import_module('plugins.' + plugin_folder_name).event_enable()
                except ModuleNotFoundError:
                    pass
                except BaseException as error:
                    return fail_api(msg="出错啦！错误信息：" + str(error))
    ...
    if plugin_folder_name:
        try:
            if plugin_folder_name in PLUGIN_ENABLE_FOLDERS:
                ...
                # 禁用插件事件
                try:
                    importlib.import_module('plugins.' + plugin_folder_name).event_disable()
                except ModuleNotFoundError:
                    pass
                except BaseException as error:
                    return fail_api(msg="出错啦！错误信息：" + str(error))
